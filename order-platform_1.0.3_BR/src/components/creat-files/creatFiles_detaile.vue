  ...
  <div class="creatFile">
    ...
    >
      <el-form-item label="收货人姓名：" prop="archive.receiverName">
        <el-input
          v-model="rulesmesinfo.archive.receiverName"
      ...
      <el-form-item
        label="收货地址："
        v-if="!rulesmesinfo.canChoiceAddress.ishow"
        class="box-cityinput"
        prop="archive.address"
      >
      ...
      <el-form-item
        label="收货地址："
        v-if="rulesmesinfo.canChoiceAddress.ishow"
        class="zeng-box-cityinput"
        prop="canChoiceAddress.value"
      >
        <el-select
          v-model="rulesmesinfo.canChoiceAddress.value"
      ...
      <el-form-item label="送货方式：" prop="delivermode.value">
        <el-select
          v-model="rulesmesinfo.delivermode.value"
          :disabled="rulesmesinfo.delivermode.disabled"
          @change="delivermodechange"
          :popper-class="rulesmesinfo.optionsClassname"
          @focus="cityselectFn"
          placeholder="请选择"
          auto-complete="off"
        >
          <el-option
            ...
            :disabled="rulesmesinfo.optionsdisabled"
            :key="item.code"
            :label="item.name"
            :value="item.code"
          >
      ...
      <el-form-item label="送货时间：" prop="delivertime.dateTime">
        ...
        <span v-if="rulesmesinfo.archive.deliverySegmentDescBegin"
          >{{ rulesmesinfo.archive.deliverySegmentDescBegin }}-{{
            rulesmesinfo.archive.deliverySegmentDescEnd
          }}</span
        >
        <span class="delivertipmes" v-if="rulesmesinfo.delivermode.message">{{
          rulesmesinfo.delivermode.message
        }}</span>
        <span
          class="songtip"
          v-if="rulesmesinfo.archive.deliveryInstallSyncFlag == 1"
          ><img :src="rulesmesinfo.archive.song"
        /></span>
      </el-form-item>
      <el-form-item
        label="安装方式："
        prop="installationmode.value"
        v-if="rulesmesinfo.delivermode.installShowFlag"
      >
        <el-select
          v-model="rulesmesinfo.installationmode.value"
      ...
      <el-form-item
        label="安装时间："
        class="installation"
        prop="installationtime.dateTime"
        v-if="rulesmesinfo.delivermode.installShowFlag"
      >
        <el-date-picker
          type="date"
        ...
        <span v-if="rulesmesinfo.archive.installSegmentDescBegin"
          >{{ rulesmesinfo.archive.installSegmentDescBegin }}-{{
            rulesmesinfo.archive.installSegmentDescEnd
          }}</span
        >
        <span
          class="songtip"
      ...
      <el-form-item
        label="不安装原因："
        class="installation"
        prop="notInstallReasonList.value"
        v-if="rulesmesinfo.delivermode.installShowFlag"
      >
        <el-select
    ...
    >
      ...
      <el-table-column prop="shippingGroupId" label="配送单号">
      </el-table-column>
      <el-table-column prop="skuNo" label="商品编码"> </el-table-column>
      <el-table-column prop="skuName"  label="商品名称">
        <template slot-scope="scope">
          <el-tag  v-if="scope.row.fullDepositFlag ==1"  type="danger" size="mini">全额订金</el-tag>{{scope.row.skuName}}
        </template>
      </el-table-column>
      <el-table-column prop="storageName" label="仓库"> </el-table-column>
      <el-table-column prop="inventoryStateDesc" label="库存状态">
      </el-table-column>
    ...
    <div class="creatFileBtn" v-if="!disabled">
      <el-button
        v-if="LOGINDATA.orderplatform_creatfiles_bao"
        type="primary"
        size="small"
        @click.stop="submitForm('rulesmesinfo')"
        >保存</el-button
      >
...
export default {
  data() {
    ...
    var receiverNamefn = (rule, value, callback) => {
      ...
      if (len > 30 && !disabled) {
        callback(
          new Error("收货人姓名最多30个字符(英文,数字,汉字各占一个字符)")
        );
      } else if (len == 0 && !disabled) {
        callback(new Error("请填写收货人姓名"));
      } else if (
        !/^[0-9a-zA-Z\u4e00-\u9fa5\.()\·\_]+$/.test(value) &&
        !disabled
      ) {
        callback(new Error("不能输入特殊字符"));
      } else if (
        len == 0 &&
        that.rulesmesinfo.delivermode.code == 1 &&
        that.rulesmesinfo.installationmode.code == 2
      ) {
        callback();
      } else {
        callback();
    ...
    var receiverPhoneNofn = (rule, value, callback) => {
      //手机验证
      //console.log(value)
      let disabled = that.rulesmesinfo.archive.disabled;
      if (that.rulesmesinfo.archive.receiverPhoneNo) {
        that.rulesmesinfo.archive.receiverPhoneNo = that.rulesmesinfo.archive.receiverPhoneNo.replace(
          /[^\d]/g,
          ""
        );
      }
      ...
      if (!value && !disabled) {
        ...
        !/^(1[3|5|4|6|7|8|9]\d{1}[*|\d]{4}\d{4})$/.test(value) &&
        !disabled
      ) {
    ...
    var receiverOtherNofn = (rule, value, callback) => {
      //其他联系方式验证
      //console.log(value)receiverOtherNo
      let disabled = that.rulesmesinfo.archive.disabled;
      if (that.rulesmesinfo.archive.receiverOtherNo) {
        that.rulesmesinfo.archive.receiverOtherNo = that.rulesmesinfo.archive.receiverOtherNo.replace(
          /[^\d|-]/g,
          ""
        );
      }
      ...
      if (!value) {
        ...
        !/^(0?(1[3|5|4|6|7|8|9]\d{1}[*|\d]{4}\d{4}))|(400|800)([0-9\\-]{7,10})|(([0-9]{4}|[0-9]{3})(-| )?)?([0-9]{7,8})((-| |转)*([0-9]{1,4}))?$/.test(
          value
        ) &&
        !disabled
      ) {
    ...
    var addressfn = (rule, value, callback) => {
      //详细地址验证
      //console.log(value)
    ...
    var delivermodefn = (rule, value, callback) => {
      //送货方式验证
      //console.log(value)
    ...
    var delivertimefn = (rule, value, callback) => {
      //送货时间验证
      // console.log(value)
    ...
    var installationmodefn = (rule, value, callback) => {
      //安装方式验证
      //console.log(value)
    ...
    var installationtimefn = (rule, value, callback) => {
      //安装时间验证
      // console.log(value)
    ...
    var notInstallReasonListfn = (rule, value, callback) => {
      //不安装方式验证
    ...
    return {
      ...
      rulesmesinfo: {
          ...
          pickerOptions: {
            ...
            disabledDate(time) {
              if (that.rulesmesinfo.delivermode.operFlag == 2) {
                return (
                  time.getTime() <
                    that.rulesmesinfo.archive.deliveryLimitBeginDate ||
                  time.getTime() >
                    that.rulesmesinfo.archive.deliveryLimitEndDate
                );
              } else {
                return (
                  time.getTime() <
                  that.rulesmesinfo.archive.deliveryLimitBeginDate || time.getTime() > that.rulesmesinfo.archive.deliveryLimitBeginDate+1000*60*60*24*365
                );
              }
            }
          }
        ...
        installationtime: {
          ...
          pickerOptions: {
            //禁用日期
            disabledDate(time) {
              // 1operFlag==2 选时间和运能 控制选择日期  2如果送货时间选择了 就以送货时间为当前时间
              //console.log(this)
              let cur = "";
              that.rulesmesinfo.delivertime.dateTime &&
              that.rulesmesinfo.delivermode.installToDeliveryFlag != 1
                ? (cur = that.rulesmesinfo.delivertime.dateTime)
                : (cur = that.rulesmesinfo.archive.installLimitBeginDate);
              if (that.rulesmesinfo.installationmode.operFlag == 2) {

                return (
                  time.getTime() < cur ||
                  time.getTime() > that.rulesmesinfo.archive.installLimitEndDate
                );
              } else {
                return time.getTime() < cur || time.getTime() > cur+1000*60*60*24*365
              }
            }
          }
        },
      ...
      rulesarchive: {
        ...
        "archive.address": [{ validator: addressfn, trigger: "blur" }],
        "canChoiceAddress.value": [
          { validator: canChoiceAddressfn, trigger: "blur" }
        ],
        "delivermode.value": [{ validator: delivermodefn, trigger: "change" }],
        "delivertime.dateTime": [{ validator: delivertimefn, trigger: "blur" }],
  ...
  methods: {
    changedability(obj) {
      ...
      if (obj.type == "delivery") {
        ...
        that.rulesmesinfo.archive.installCompanyCodecur =
          obj.installCompanyCode;
        that.rulesmesinfo.archive.sayt = obj.sayt;
        that.rulesmesinfo.archive.installSegmentCode = obj.installSegmentCode;
        that.rulesmesinfo.archive.installSegmentCodecur =
          obj.installSegmentCode;
        //对应的安装能的code
        ...
        if (obj.st == "close") {
          ...
          if (that.rulesmesinfo.delivermode.installToDeliveryFlag == 1) {
            // 如果是售后带货安装的话  安装时间要反写到配送时间
            that.rulesmesinfo.delivertime.dateTime = "";
          }
          return false;
        }
        ...
        if (valid) {
          ...
          let filesobj = {
            ...
            archive: {
              ...
              deliverySegmentCode:
                that.rulesmesinfo.archive.deliverySegmentCode, //运能对应的code
              deliverySegmentDescBegin:
                that.rulesmesinfo.archive.deliverySegmentDescBegin, //运能对应的开始时间
              deliverySegmentDescEnd:
                that.rulesmesinfo.archive.deliverySegmentDescEnd, //运能对应的结束时间
              ...
              installSegmentDescBegin:
                that.rulesmesinfo.archive.installSegmentDescBegin, //安装能对应的开始时间
              installSegmentDescEnd:
                that.rulesmesinfo.archive.installSegmentDescEnd, //安装能对应的结束
    ...
    changeCity(val) {
      ...
      that.initobject.countyCode = that.cityobj.aeracode3.code;
      that.initData(that.initobject, 1);
      // console.log(that.cityobj)
    },
    canChoiceAddresschange(key) {
      //选择完多个地址
      let that = this;
      //console.log(key)
    ...
    cityselectFn(callback, val) {
      if (!this.cityobj.aeracode4.code) {
        this.$message.error("请先选择送货区域");
        return false;
      }

      return false;
    },
    notInstallReasonchange(key) {
      //不按装select选中事件
    ...
    initData(initobject, cityinfo) {
      ...
      API.queryGoodsItemArchive(initobject).then(function(data) {
        ...
        if (data && data.success && data.response) {
          ...
          if (that.itemData.length > 1) {
            that.$message({
              showClose: true,
              message:
                "因配送单中含有多件小件商品，改档时整个配送单商品一起改档，不支持某件商品单独改档",
              type: "warning",
              duration: 0
            });
          }
          ...
          //不安装原因
          that.rulesmesinfo.notInstallReasonList.options = initdata.initResult
            .notInstallReasonList
            ? initdata.initResult.notInstallReasonList
            : [];
          //送货方式
          that.rulesmesinfo.delivermode.options = initdata.initResult
            .canChoiceDeliveryMethodList
            ? initdata.initResult.canChoiceDeliveryMethodList
            : [];
          // 处理送货方式和安装方式的联动
          //初始化页面信息展示
          ...
          that.rulesmesinfo.delivertime.dateTime =
            initdata.archive.deliveryDate;
          that.rulesmesinfo.delivermode.disabled = that.initFlag(
            initdata.archive.deliveryFlag
          ); //禁止送货方式

          //2 安装时间
          that.initInstall("isSelected", 1, initdata.archive);
          that.rulesmesinfo.installationtime.dateTime =
            initdata.archive.installDate;
          that.rulesmesinfo.installationmode.disabled = that.initFlag(
            initdata.archive.installFlag
          ); //禁止安装方式
          //处理 首次填写地址的时候需要不清除的内容
          that.rulesmesinfo.archive.receiverName
            ? (initdata.archive.receiverName =
                that.rulesmesinfo.archive.receiverName)
            : ""; //收货人姓名
          that.rulesmesinfo.archive.receiverPhoneNo
            ? (initdata.archive.receiverPhoneNo =
                that.rulesmesinfo.archive.receiverPhoneNo)
            : ""; //收货人手机号
          that.rulesmesinfo.archive.receiverOtherNo
            ? (initdata.archive.receiverOtherNo =
                that.rulesmesinfo.archive.receiverOtherNo)
            : ""; //其他联系方式
          that.rulesmesinfo.archive.address
            ? (initdata.archive.address = that.rulesmesinfo.archive.address)
            : ""; //详细地址
          that.rulesmesinfo.archive = initdata.archive;
          that.rulesmesinfo.archive.installQueryType =
            that.rulesmesinfo.installationmode.code == 1 ? 1 : 0;

          //初始化时间展示
          if (cityinfo) {
            //通过选择完地址之后的操作

            that.rulesmesinfo.archive.stateCode = null;
            that.rulesmesinfo.delivermode.value = "";
            that.rulesmesinfo.delivermode.code = "";
          ...
          // that.cityobj.aeracode4.firstaera = true;
          //初始化没有地址 下拉不展示optionsClassnameN
          if (!that.cityobj.aeracode4.code) {
            //记录初始化没有地址
            that.cityobj.aeracode4.firstaera = false;

            that.rulesmesinfo.optionsdisabled = true;
            that.rulesmesinfo.optionsClassname = "optionsClassnameN";
          } else {
            that.rulesmesinfo.optionsdisabled = false;
            that.rulesmesinfo.optionsClassname = "";
          }

          //初始化范围(收货人姓名、手机、联系方式)置灰不让点击
          //initdata.archive.receiveFlag=0
          that.rulesmesinfo.archive.disabled = that.initFlag(
            initdata.archive.receiveFlag
          );
          //详细人地址可否修改
          // console.log(initdata.archive.addressFlag)
          that.rulesmesinfo.archive.addressdisab = that.initFlag(
            initdata.archive.addressFlag
          );
          //console.log(initdata.archive.addressFlag)

          if (
            !(that.initFlag(initdata.archive.cityAddressFlag) || that.disabled)
          ) {
            // 据说这个页面要修该 也只能修改三级
            // 允许修改四级区域地质
            let cityBloo = true;
            that.cityobj.curstyle = "";
            if (!that.cityobj.aeracode4.firstaera) {
              cityBloo = false;
            }

            that.cityobj.aeracode1.disable = cityBloo;
            that.cityobj.aeracode2.disable = cityBloo;
            that.cityobj.aeracode3.disable = false;
            that.cityobj.aeracode4.disable = false;
            //如果允许修改三四级 需要请求接口传值商品唯一号和配送单号
            that.cityobj.aeracode4.commerceItemId =
              that.itemData[0].commerceItemId;
            that.cityobj.aeracode4.shippingGroupId =
              that.itemData[0].shippingGroupId;
          }

          //赠品
          if (
            initdata.archive.canChoiceAddressList &&
            initdata.archive.canChoiceAddressList.length >= 1
          ) {
            that.rulesmesinfo.canChoiceAddress.ishow = true;
            that.rulesmesinfo.canChoiceAddress.options =
              initdata.archive.canChoiceAddressList;
            //console.log(initdata)
            if (
              initdata.archive.cityAddressFlag == 1 &&
              initdata.archive.addressFlag == 1
            ) {
              //支持修改四级区域和详细地址
          ...
          //区域是否支持送按一体
          that.rulesmesinfo.archive.sayt =
            initdata.archive.deliveryInstallAreaSyncFlag;
          ...
          if (that.operStatu == 0) {
            ...
            //售后带货安装特殊文字提示“此送货时间为送到网点的时间，到家以安装时间为准！”
            if (
              that.rulesmesinfo.delivermode.code == '3' &&
              that.rulesmesinfo.delivermode.installShowFlag == 1 &&
              !that.rulesmesinfo.installationtime.disabled
            ) {
    ...
    initDeliver(isSelected, code, archive) {
      //取对应的送货参数
      let that = this;
      for (let i = 0; i < that.rulesmesinfo.delivermode.options.length; i++) {
        if (that.rulesmesinfo.delivermode.options[i][isSelected] == code) {
          that.rulesmesinfo.delivermode.value =
            that.rulesmesinfo.delivermode.options[i].name; //送货当前展示
          that.rulesmesinfo.delivermode.code =
            that.rulesmesinfo.delivermode.options[i].code; //送货当前code
          that.rulesmesinfo.delivermode.operFlag =
            that.rulesmesinfo.delivermode.options[i].operFlag; //送货运能调用
          that.rulesmesinfo.delivermode.installToDeliveryFlag =
            that.rulesmesinfo.delivermode.options[i].installToDeliveryFlag; // 是否安装能赋值运能
          that.rulesmesinfo.installationmode.options = that.rulesmesinfo
            .delivermode.options[i].canChoiceInstallMethodList
            ? that.rulesmesinfo.delivermode.options[i]
                .canChoiceInstallMethodList
            : [];
          that.resetinstall(1);
          ...
          if (
            that.rulesmesinfo.delivermode.operFlag == 0 ||
            archive.deliveryDateFlag == 0
          ) {
            //0送货日期不可选 1选日期不调接口 2选日期调接口
            //alert(2)
          ...
          if (
            that.rulesmesinfo.delivermode.code == '3' &&
            that.rulesmesinfo.delivermode.installShowFlag != 0 &&
            !that.rulesmesinfo.installationtime.disabled
          ) {
            //如果售后带货安装提示
            that.rulesmesinfo.delivermode.message =
        ...
        that.rulesmesinfo.delivermode.installShowFlag =
          that.rulesmesinfo.delivermode.options[i].installShowFlag;
      }

      that.$refs["rulesmesinfo"].validate();
    },
    resetinstall(s, isSelected) {
      // 修改送货方式以及送货时间的处理
      let that = this;

      if (!that.rulesmesinfo.delivertime.disabled) {
        //时间禁止选择的时候不变
      ...
      if(!that.rulesmesinfo.installationmode.disabled){
      ...
      that.rulesmesinfo.installationmode.deliveryToInstallFlag = 0;
    ...
    initInstall(isSelected, code, archive) {
      ...
      for (
        let i = 0;
        i < that.rulesmesinfo.installationmode.options.length;
        i++
      ) {
        if (that.rulesmesinfo.installationmode.options[i][isSelected] == code) {
          that.rulesmesinfo.installationmode.value =
            that.rulesmesinfo.installationmode.options[i].name; //安装当前展示
          that.rulesmesinfo.installationmode.code =
            that.rulesmesinfo.installationmode.options[i].code; //安装当前code
          that.rulesmesinfo.installationmode.operFlag =
            that.rulesmesinfo.installationmode.options[i].operFlag; //安装运能调用
          that.rulesmesinfo.installationmode.deliveryToInstallFlag =
            that.rulesmesinfo.installationmode.options[i].deliveryToInstallFlag;
          //是否运能赋值安装能
          if (
            that.rulesmesinfo.installationmode.operFlag == 0 ||
            archive.installDateFlag == 0
          ) {
      ...
      if (that.rulesmesinfo.installationmode.code == 2) {
        ...
        if (that.rulesmesinfo.delivertime.dateTime > 0) {
          // 增加座装和挂装
          that.addSeathanger();
        }
        that.rulesmesinfo.notInstallReasonList.disabled = true;
    ...
    songan(isSelected) {
      ...
      if (
        that.rulesmesinfo.archive.able == 3 &&
        that.rulesmesinfo.installationmode.code == 1
      ) {
        ...
        that.rulesmesinfo.archive.installSegmentDescBegin =
          that.rulesmesinfo.archive.deliverySegmentDescBegin; //安装能重选
        that.rulesmesinfo.archive.installSegmentDescEnd =
          that.rulesmesinfo.archive.deliverySegmentDescEnd;
        that.rulesmesinfo.archive.installSegmentCode =
          that.rulesmesinfo.archive.installSegmentCodecur;
        that.rulesmesinfo.archive.installSite =
          that.rulesmesinfo.archive.installSitecur;
        that.rulesmesinfo.archive.installArea =
          that.rulesmesinfo.archive.installAreacur;
        that.rulesmesinfo.archive.installCompanyCode =
          that.rulesmesinfo.archive.installCompanyCodecur;
      } else {
        if (isSelected == "code") {
    ...
    initunInstall() {
      ...
      for (
        let i = 0;
        i < that.rulesmesinfo.notInstallReasonList.options.length;
        i++
      ) {
        if (that.rulesmesinfo.notInstallReasonList.options[i].isSelected == 1) {
          ...
          that.rulesmesinfo.notInstallReasonList.value =
            that.rulesmesinfo.notInstallReasonList.options[i].name; //安装当前展示
          that.rulesmesinfo.notInstallReasonList.code =
            that.rulesmesinfo.notInstallReasonList.options[i].code; //安装当前code
        }
      }
    },
    delivertimeChang(s) {
      ...
      let that = this;
      if (
        that.rulesmesinfo.installationtime.dateTime * 1 &&
        that.rulesmesinfo.delivertime.dateTime * 1 >=
          that.rulesmesinfo.installationtime.dateTime * 1
      ) {
        that.rulesmesinfo.installationtime.dateTime = "";
      }
      if (that.rulesmesinfo.installationmode.deliveryToInstallFlag != 1) {
    ...
    installationChang(s) {
      ...
      if (
        s * 1 < that.rulesmesinfo.delivertime.dateTime * 1 &&
        that.rulesmesinfo.delivermode.installToDeliveryFlag != 1
      ) {
