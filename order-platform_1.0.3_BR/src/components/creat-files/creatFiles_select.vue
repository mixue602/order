  ...
  <div class="container">
    ...
    <div class="tit-box">
      ...
      >
        ...
        <el-form-item label="配送单号：" prop="deliveryId">
          <el-input
            style="width: 240px;"
            v-model="labelSearch.deliveryId"
            placeholder="请输入配送单号"
            maxlength="20"
            auto-complete="off"
            size="mini"
        ...
        <el-form-item label="订单状态：" prop="orderStatus">
          <el-select
            style="width: 240px;"
            v-model="labelSearch.orderStatus.value"
            placeholder="请选择"
            size="mini"
          >
            <el-option
        ...
        <el-form-item
          label="选择门店："
          v-show="labelSearch.closeshop.isdata == 1"
          v-if="LOGINDATA.orderplatform_creatfiles_closeshop"
        >
          <g-closeshop
            style="width: 240px;"
            v-model="labelSearch.closeshop"
            placeholder="请选择"
          ></g-closeshop>
        </el-form-item>
        <el-form-item label="下单时间：" prop="dateEvnet" style="width:600px;">
          <el-date-picker
            ...
            type="date"
            placeholder="选择开始日期"
            v-model="labelSearch.startDate"
            value-format="yyyy-MM-dd"
            :picker-options="startDateEvent"
            size="mini"
            auto-complete="off"
          ></el-date-picker
          ...
          <el-date-picker
            ...
            type="date"
            placeholder="选择结束日期"
            v-model="labelSearch.endDate"
            value-format="yyyy-MM-dd"
            :picker-options="endDateEvent"
            size="mini"
            auto-complete="off"
          ></el-date-picker>
        </el-form-item>
    ...
    <div class="table-box">
      <el-table
        ...
        :default-sort="{ prop: 'list.orderId', order: 'list.descending' }"
        style="width: 100%"
      >
        <el-table-column
          prop="orderId"
        ...
        <el-table-column prop="skuNo" sortable label="商品编码">
        </el-table-column>
        ...
        <el-table-column fixed="right" prop="status" label="操作">
          <template slot-scope="scope">
            <el-button
              class="scope-btn1"
              v-if="
                scope.row.queryFlag == 1 &&
                  LOGINDATA.orderplatform_creatfiles_explorer
              "
            ...
            <el-button
              ...
              v-if="
                scope.row.updateFlag == 1 &&
                  LOGINDATA.orderplatform_creatfiles_modify
              "
...
.tit-box .el-form-item.categorybox {
  width: 100%;
}
...
export default {
  data() {
    ...
    var receiverOrderIdfn = (rule, value, callback) => {
      that.labelSearch.orderId = that.labelSearch.orderId.replace(
        /(^\s*)|(\s*$)/g,
        ""
      );
    ...
    var receiverVipIdfn = (rule, value, callback) => {
      that.labelSearch.vipId = that.labelSearch.vipId.replace(
        /(^\s*)|(\s*$)/g,
        ""
      );
    ...
    var receiverDeliveryIdfn = (rule, value, callback) => {
      that.labelSearch.deliveryId = that.labelSearch.deliveryId.replace(
        /(^\s*)|(\s*$)/g,
        ""
      );
    ...
    var receiverNamefn = (rule, value, callback) => {
      that.labelSearch.name = that.labelSearch.name.replace(
        /(^\s*)|(\s*$)/g,
        ""
      );
    ...
    var receiverPhonefn = (rule, value, callback) => {
      that.labelSearch.phone = that.labelSearch.phone.replace(
        /(^\s*)|(\s*$)/g,
        ""
      );
    ...
    return {
      ...
      selectrules: {
        orderId: [{ validator: receiverOrderIdfn, trigger: "blur" }],
        vipId: [{ validator: receiverVipIdfn, trigger: "blur" }],
        deliveryId: [{ validator: receiverDeliveryIdfn, trigger: "blur" }],
        // salesId: [
        // { validator: receiverSalesIdfn, trigger: 'blur' }
        // ],
        name: [{ validator: receiverNamefn, trigger: "blur" }],

        phone: [{ validator: receiverPhonefn, trigger: "blur" }],

        dateEvnet: [{ validator: receiverDateFn, trigger: "blur" }]
      },
      startDateEvent: {
        disabledDate(time) {
          if (
            that.labelSearch.endDate != "" &&
            that.labelSearch.endDate != null
          ) {
      ...
      endDateEvent: {
        disabledDate(time) {
          ...
          if (
            ...
            that.labelSearch.startDate != null
          ) {
  ...
  activated() {
    ...
    if (
      cacheSearch.orderId != "" ||
      cacheSearch.vipId != "" ||
      cacheSearch.deliveryId != "" ||
      cacheSearch.name != "" ||
      cacheSearch.phone != "" ||
      (cacheSearch.startDate != "" && cacheSearch.endDate != "")
    ) {
      this.getCacheSelect();
    }
  },
  ...
  methods: {
    ...
    getSelect() {
      ...
      if (this.categoryprop.isCheckMyself) {
        //主任选择仅查看自己的品类
        that.labelSearch.guiderCode = this.LOGINDATA.employeeId;
      } else {
        that.labelSearch.guiderCode = "";
      }
      if (
        (that.labelSearch.orderId != "" ||
          that.labelSearch.vipId != "" ||
          that.labelSearch.deliveryId != "" ||
          that.labelSearch.name != "" ||
          that.labelSearch.phone != "" ||
          that.labelSearch.orderStatus.value != "" ||
          (that.labelSearch.startDate != "" &&
            that.labelSearch.endDate != "") ||
          that.labelSearch.closeshop.value != "" ||
          that.categoryprop.categoryCode != null ||
          that.labelSearch.guiderCode != "") &&
        that.select
      ) {
        that.pageData = {
        ...
        var parms = {
          ...
          skuCategoryIds: that.categoryprop.categoryCode
        };
        if (this.categoryprop.isCheckMyself) {
          //主任选择仅查看自己的品类
          parms.guiderCode = this.LOGINDATA.employeeId;
        }
        API.queryArchiveSelect(parms).then(function(data) {
