                ...
                <el-row class="as-info as-processing__caption" data-before="退换货信息">
                    ...
                    <el-row class="refundExchangeAddress">
                        ...
                        <el-row class="rea__item rea__ogistics">
                            <el-row class="exchanger__info" v-if="(serviceData.returnType == '2' || form.returnType =='2' || form.returnType =='4')">
                                <el-form-item label="新安装方式：" prop="installation">
                                    <el-select v-model="form.installation" placeholder="请选择" @change="installationmodechange">
                                        <el-option v-for="(item, index) in supportInstallMethodList"
                                                :label="item.installMethodName"
                                                :value="item.installMethodCode"
                                                :key="index">
                                            {{item.installMethodName}}
                                        </el-option>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="不安装原因：" v-if="form.installation == 2" prop="installation">
                                    <el-select v-model="form.notinstallreason" placeholder="请选择">
                                        <el-option v-for="(item, index) in donotInstallReasonList"
                                                :label="item"
                                                :value="item"
                                                :key="index">
                                            {{item}}
                                ...
                                <el-form-item label="新安装时间：" prop="installDate" v-if="form.installation != 2">
                                    <div class="block">
                                        <el-date-picker
                                                ...
                                                :disabled="(form.pickupDate =='' || form.operFlag == 0) ? true : false">
                                        </el-date-picker>
    ...
    export default {
        data () {
            ...
            return {
                ...
                serviceData: {
                    ...
                    supportInstallMethodList:[]
                },
                supportInstallMethodList:[
                    {installMethodCode:1,installMethodName:'售后安装',operFlag:2,isSelected:0},
                    {installMethodCode:2,installMethodName:'不安装',operFlag:0,isSelected:1},
                    {installMethodCode:3,installMethodName:'厂家安装',operFlag:1,isSelected:0},
                    {installMethodCode:4,installMethodName:'电话预约',operFlag:0,isSelected:0},
                    {installMethodCode:5,installMethodName:'座装',operFlag:0,isSelected:0},
                    {installMethodCode:6,installMethodName:'挂装',operFlag:0,isSelected:0},
                ],
                donotInstallReasonList:[
                    '顾客自行安装','外地用户','超出安装范围','对公用户','同型号换货','门店倒票','其他'
                ],
                returnInstallInfo:{
                    installMethod:2,
                },
        ...
        methods: {
            installationmodechange(val){
                ...
                that.initInstall('installMethodCode',val);
            },
            initInstall(name,val){
                var that =  this;
                for(let i=0; i<that.supportInstallMethodList.length;i++){
                    if(that.supportInstallMethodList[i][name]==val){//选取当前选中值对应的数据

                        that.form.installationName = that.supportInstallMethodList[i]['installMethodName'];
                        that.form.operFlag = that.supportInstallMethodList[i]['operFlag'];
                        that.form.installation = that.supportInstallMethodList[i]['installMethodCode'];
                    }
                };
                if(that.form.installationName =='座装' || that.form.installationName =='挂装'){//运能赋值安装能
            ...
            commonInit (response,type) {
                if (response.success) {
                    if (response.response) {
                        ...
                        //安装方式列表
                        this.initInstall('isSelected',1);                                                                                        //顾客信息
                    } else {
                        this.loading = false;
                    }
                } else {
            ...
            changePickupDate () {
                ...
                if(_this.form.installationName =='座装' || _this.form.installationName =='挂装'){//运能赋值安装能
                    _this.form.installDate = _this.form.pickupDate;
                }
            },
            //选择送货能时间段
                ...
                if (obj.type == "delivery") {//送货能返回
                    ...
                    this.installpickerOptions = {
                        disabledDate(time) {
                            ...
                            let endTime = new Date(startTime);
                            endTime.setDate(startTime.getDate() + 53);
                            return time.getTime() < startTime.getTime() || time.getTime() > endTime.getTime();
                        }
                    }
                }
            ...
            changeInstallDate () {
                ...
                // };
                _this.initInstall('installMethodCode',_this.form.installation);
            ...
            saveReturnRequest (ruleForm) {
                ...
                let params = {
                    returnRequestAgentDto: {
                        ...
                        memberCardNo: this.serviceData.userInfoDto.memberCardNo, 
                                                                                                                                              //会员卡号
                    }
                };
                let url = window.location.href.split('?')[1];
                ...
                //退换货提交保存
                    ...
                    alert('2')
                    params.returnRequestAgentDto.returnInstallAbility={//安装运能信息
                         installMethod:this.form.installation,//安装方式
                         donotInstallReasonName:this.form.notinstallreason,//不安装原因中文描述   
                         installSite:this.form.installSite,//安装网点 
                         installAreaCode:this.form.installArea,//安装区域
                         saleCompanyCode:this.form.installSegmentCode,//售后公司代码installCompanyCode
                         installDate:this.form.installDate,//安装日期
                         installCapacityCode:this.form.installSlot,//安装时段编号
