...
<div class="order_detailsbox">
    <div class="order_details_l"> 
        <el-collapse v-model="pageactiveNames" class="orderdetails_title" @change="handleChange">
                            ...
                            <div class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition el-table--small" style="overflow-x:auto;">   
                                <table cellspacing="0" cellpadding="0" border="0" class="el-table__header" width="1700">
                                    ...
                                    <tbody class="el-table__body" v-for="(prdItem,index) in shippingItem.commerceItemList" :key="prdItem.id">
                                        <tr class="el-table__row">
                                            <td class="is-center">
                                                <i v-if='prdItem.salesPromotionList.length !=0 || prdItem.remark || prdItem.storeEmpId || prdItem.cardList.length !=0' class="shippingbtn_icon" @click="prdItem.show=!prdItem.show" v-bind:class="prdItem.show ? 'el-icon-remove-outline' : 'el-icon-circle-plus-outline'"></i>
                                            </td>
                                            <td class="is-center">
                                        ...
                                        <tr class="el-table__row" style="background:#EBEEF5;" v-if='(prdItem.show && prdItem.salesPromotionList.length !=0) || (prdItem.show && prdItem.remark) || (prdItem.show && prdItem.storeEmpId) || (prdItem.show && prdItem.cardList.length !=0)'>
                                            <td colspan="17">
                                                <div class="ml40" v-if='prdItem.show && prdItem.salesPromotionList.length !=0'>
                                                    <div class="paymentmesg" v-for="salesItem in prdItem.salesPromotionList" :key="salesItem.id"><span class="mr20">{{salesItem.promotionName}}：{{salesItem.promotionDesc}}</span><span v-if="salesItem.giftCouponItemNum">x {{salesItem.giftCouponItemNum}}</span></div>
                                                    ...
                                                    <div class="paymentmesg1">
                                                        ...
                                                        <div>
                                                            <p v-for="(carditem,index) in (prdItem.cardList)" :key="carditem.id">
                                                                <span v-if="carditem.cardType == '0'">蓝卡</span>
                                                                <span v-else>机价补贴卡</span>
                                                                <span>价值：{{carditem.cardAmount}}</span> 
                                                                <span>关联卡号：{{carditem.cardMobileNo}}</span>
                                                            </p>
                                                        </div>
                                                    </div>                         
                                                </div>
            ...
            <el-collapse-item title="运营商使用卡信息" name="5">
                <!-- 运营商使用卡信息 -->    
    ...
    <div class="order_details_r">
        ...
        <div class="payinfor">
            ...
            <!-- 运营商 -->
            <div class="item" v-if="response.useBuleCardAmount">
                <span class="label">使用蓝卡：</span> 
                <div class="info-rcol">- ¥{{response.useBuleCardAmount}}</div> 
            </div>
            <div class="item mb10" v-if="response.useAllowanceCardAmount">
                ...
                API.orderdetailsbeforesplit(parms).then(function(data) {
                    if(data.success && data.response !=null){
                        ...
                        data.response.useCardList=[
                            {cardType:0,cardAmount:"34.00",cardMobileNo:'13425263521'},
                            {cardType:1,cardAmount:"134.00",cardMobileNo:'13425263521'}, 
                        ]                
