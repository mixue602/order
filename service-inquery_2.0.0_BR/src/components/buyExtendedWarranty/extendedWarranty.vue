          ...
          data.incrementSkuInfoList.forEach((item, j) => {
            if (item.isSelected) {
              var paramInfo = {
                ...
                canBuyWarrantyQuantity: item.canBuyWarrantyQuantity
              };
      ...
      API.AddSellerBill(param).then(data => {
        if (data.success) {
          ...
          var newUrl =
            "//mpf" +
            Env.cookieDomain +
            "/service/purchaseExtension/?memberId=" +
            data.response.memberId +
            "&memberCard=" +
            data.response.memberCard +
            "&temporaryCardFlag=" +
            data.response.temporaryCardFlag +
            "&siteType=" +
            data.response.siteType +
            "&sellerNum=" +
            data.response.sellerNum +
            "&storeCode=" +
