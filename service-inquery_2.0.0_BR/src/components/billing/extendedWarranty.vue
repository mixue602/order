<template>
  <div class="extended-warranty-box">
    <div class="preferential-box">
      <div class="preferential-tip">延保：</div>
      
      <div class="preferential-content">
        <span v-if="pGoodsData&&pGoodsData.loadIncrementInfo&&pGoodsData.loadIncrementInfo.incrementFlag && (!pGoodsData.loadIncrementInfo.checkedIncrementInfo)" class="click-btn" @click="isAllowClick"><i class="el-icon-circle-plus"></i></span>
        <p class="selected-show" @click="isAllowClick">{{selectedStr}}</p>
      </div>
    </div>
    <el-dialog 
            ...
            <div class="list-box" v-for="(data,index) in extendData.showGroupList" :key="index">
              ...
              <ul class="clearfloat" v-if="data && data.incrementSkuInfoList.length>0">
                <li class="list" 
                  ...
                  <span v-show="isShowTj" v-on:click.stop class="tj" >TJ {{list.rewardPrice | clearTailZero}}
                  </span>
...
import "@/common/tofixed";
  ...
  export default {
    ...
    data() {
      return {
        dialogVisible: this.pDialogVisible || false,
      ...
      selectedStr(){
        ...
        if( this.checkedList.length>0){
          this.checkedList.forEach((item,idx)=>{
              var str = '';
              str+= item.displaySkuName + item.years +'年 ￥'+item.incrementPrice+' ×'+item.quantity;
    ...
    watch:{
      ...
      pDialogVisible: {
        handler:function(){
          this.dialogVisible =  this.pDialogVisible || false;
        }
      }
    ...
    methods: {
      ...
      dataInit(){
           ...
           if(!this.pGoodsData.loadIncrementInfo.unCheckedIncrementInfo) {
             return false;
           }
      ...
      bindConfirm() {
          var param = JSON.parse(JSON.stringify(this.$store.state.billingUsedParam));
              ...
              param.addToBillIncrement = {
                commerceItemId: this.pGoodsData.commerceItemId,//商品ID
                increments:this.selectedData
              };

          API.addIncrementItemToBillOrCart(param).then((data) => {
          if(data.success) {
            this.initOrder(this);
      ...
      //是否可点击
      isAllowClick(){
        if(!this.$store.state.billingEdit){ //开单页 
