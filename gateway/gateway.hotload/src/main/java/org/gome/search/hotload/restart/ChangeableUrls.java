...
import org.gome.search.hotload.settings.DevToolsSettings;
...
final class ChangeableUrls implements Iterable<URL> {
	...
	private ChangeableUrls(URL... urls) {
		...
		for (URL url : urls) {
            if(!settings.isRestartExclude(url) && (settings.isRestartInclude(url) || isFolderUrl(url.toString())))
                reloadableUrls.add(url);
            if(url.toString().contains("starter-dubbo"))
                reloadableUrls.add(url);
		}
