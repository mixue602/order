...
/***/ (function(module, exports, __webpack_require__) {
	...
	if(pageData.isResult){
	    __webpack_require__(41);//右侧店铺精选和底部的推广商品
	    __webpack_require__(19).getData("#prdRight-2");//右侧热销推荐
	    __webpack_require__(42).getData("#prdRight-3");//右侧搜索了还购买了
	    __webpack_require__(43).getData("#prdRight-4");//右侧底部推荐活动（图片）
	    __webpack_require__(44).getData("#prdBottom-4");//页面底部推荐活动（图片）
	    __webpack_require__(45).getReleQuery(pageData.searchkey);//底部相关搜索词推荐
	}else{
	    if(!pageData.isfilter){//当不是筛选结果页的时候
	        __webpack_require__(22).getData("#prdBottom-1");//无结果情况底部热销推荐
	    }else{//是筛选结果的时候左侧展示热销推荐，下面展示推广商品
	        __webpack_require__(19).getData("#prdRight-2");//右侧热销推荐
	        __webpack_require__(41);//右侧店铺精选和底部的推广商品（隐藏店铺精选）
	        $("#prdRight-1").remove();
	    }
	}
	...
	document.getElementById("lazyajaxloadarea").onmouseover = function (event) {
	    ...
	    __webpack_require__(46).getData("#prdBottom-2");//猜你喜欢
...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(".filter-resultSearch-btn").bind("click", function(e) {
	    ...
	    __webpack_require__(6).dofacet('et',searchVal);
	});
