...
/***/ (function(module, exports, __webpack_require__) {
	...
	//引入大数据推荐的模块
	__webpack_require__(25).getData();//顶部云眼推荐商品
	__webpack_require__(31);//右侧店铺精选和底部的推广商品
	__webpack_require__(28).getData("#prdRight-2");//右侧一周销量排行广商品
	__webpack_require__(32).getData("#prdRight-3");//右侧浏览了还购买商品
	__webpack_require__(33).getData("#prdRight-4");//右侧购买了还购买商品
	...
	function getData(){
	    $.ajax({
	        ...
	        if(data.products.length>3){
	            ...
	            if(isHyg){
	                __webpack_require__(26).getData("#hot-list")
	            }else{
	                __webpack_require__(27).getData("#hot-list")
	            }
	        }
	    ...
	    $.get(
	        ...
	        function(data){
	            if (data.lst && data.lst.length > 0) {
	                data.lst.splice(6,data.lst.length)
	                var listTpl = templateSimple.compile(tpl_normal)(data);
	                $(domId).append('<div class="prd-right-normal"><h3 class="hd">购买此类商品的用户还购买了</h3><ul class="bd" id="bigD_liulan">'+listTpl+'</ul></div>');
	            }
	        },
	        "jsonp"
	    );
