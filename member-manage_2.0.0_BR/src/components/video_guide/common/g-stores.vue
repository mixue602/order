      ...
      //获取选择的部门
			getCheckedNodes() {
				...
				let oThis = this,
						nodes = [],
						checkedNodes = [];
						oThis.form.childStoreCodes = [];
						oThis.checkedAll = false;

				//如果存在只选一个二级品类的时候，将对应的一级品类也返回
				oThis.$nextTick(() => {
						if (oThis.$refs.tree !== undefined) {
							nodes = oThis.$refs.tree.getCheckedNodes();
						}else {//多设置一棵树的原因就是为了默认全选
							nodes = oThis.$refs.treenone.getCheckedNodes();
						}
						...
						//将获取到的id传给父组件，给查询时使用
						let data = {
							storeCodes: oThis.form.childStoreCodes,
						};
						oThis.doThis();
						if(!oThis.$refs.tree) {
							oThis.$emit('changeend',data);
							oThis.checkedTreeItems = [];
						}else {
								//将重复的部门过滤
							oThis.checkedTreeItems = checkedNodes.filter(function(element,index,self){
								return self.indexOf(element) === index;
							});
						}

						if(!oThis.$refs.tree) {
							oThis.defaultCode = oThis.form.childStoreCodes;
							oThis.defaultCheckedTreeItems = oThis.checkedTreeItems;
			...
			setCheckedKeys() {
				...
				this.$nextTick(() => {
					...
					_this.form.childStoreCodes = _this.defaultCode;
					_this.checkedTreeItems = _this.defaultCheckedTreeItems;
					let data = {
						storeCodes: _this.form.childStoreCodes,
					};
					_this.doThis();	
				});
			},
