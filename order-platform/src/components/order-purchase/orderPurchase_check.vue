  ...
  <div> 
      ...
      <el-row class="order_search_box">
        <el-form  :inline="true" :model="form" :rules="rules" ref="form"  label-width="180px" size="mini" >
            ...
            <el-row>
                <el-col  :span="24">
                    <el-form-item label="加购时间：" class="is-required" v-bind:class="{iserror:timeError}">
            ...
            <div class="c_grey">注：标<span style="color:#f56c6c">*</span>号的查询条件，必须填写两项</div>   
        </el-form>
        </el-row>
        <el-row>
        <!-- 查询导购单列表 -->
        ...
        <el-table :data="tableData" border  style="width:100%" :show-header="true" >
            ...
            <el-table-column prop="storeName" label="销售部门" align="center" width="275"></el-table-column>
            <el-table-column prop="memberCardNo" label="会员卡号" align="center" width="200"></el-table-column>
            <el-table-column prop="guiderOrderQuantity" label="商品数量" align="center" width="100"></el-table-column>
            <el-table-column prop="guiderOrderMoney" label="销售金额" align="center" width="150"></el-table-column>
            <el-table-column prop="guiderCode" label="导购员编号" align="center" width="230"></el-table-column>
            <el-table-column prop="guiderName" label="导购员" align="center" width="230"></el-table-column>
            <el-table-column prop="guiderOrderTime" label="加购时间" align="center"  width="200">
                <template slot-scope="scope">{{scope.row.guiderOrderTime | formatDate}}</template>
            </el-table-column>
            <el-table-column  label="操作" align="center" width="150">
                <template slot-scope="scope">    
                    <el-button type="text" size="small" @click.native.prevent="dialogFormVisible = true, rowsindex=scope.$index" v-if="LOGINDATA.orderplatform_orderpurchase_delate">删单</el-button>
  ...
  export  default {
    data() {
      ...
      return {
        ...
        rules: {//校验规则
                memberCardNo:[//
                    { required: true,validator: validateMemberCardNo, trigger: 'change' }
                ],//sellerNo
                guiderCode:[//导购员编号
                    {  required: true,validator: validateGuiderCode, trigger: 'change' }
                ],
                skuNo:[//商品编号
    ...
    methods:{
          ...
          var data={
            ...
            if(val){
                ...
                if(!this.card && !this.timeError && !this.guider && !this.skuNo){
                    if((memberCardNo!='' && guiderCode!='') || (memberCardNo!='' && (beginTime!='' && endTime!='')) ||(guiderCode!='' && (beginTime!='' && endTime!=''))){
                        that.orderlist(data)
                    }else{
                        Message.warning('带*号查询条件必填两项');
                    }
                }
            } 
      },
      ...
      delateForm(){
          ...
          API.deleteGuiderOrder({"guiderOrderId":guiderOrderId,"guiderCode":guiderCode}).then(function(data){
            if(data.success){
                setTimeout(function(){ 
