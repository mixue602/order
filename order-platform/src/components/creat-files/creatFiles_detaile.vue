            ...
            <el-form-item label="送货方式：" prop="delivermode.value" >
             ...
             <el-form-item label="送货时间：" prop="delivertime.dateTime" >
                 <el-date-picker
                  ...
                  <span class="delivertipmes">{{rulesmesinfo.delivermode.message}}</span>
            </el-form-item>
            <el-form-item label="安装方式：" prop="installationmode.value"        
             v-if="rulesmesinfo.archive.installShowFlag">
  ...
  export  default {
    data() {
        ...
        var receiverNamefn=(rule, value, callback) => { //姓名验证
               ...
               // console.log(len)
                if(len==0 && that.rulesmesinfo.delivermode.code==1 && that.rulesmesinfo.installationmode.code==2){
                    callback();
                }else if(len > 30 && !disabled){
                    callback(new Error("收货人名称最多30个字符(英文,数字,汉字各占一个字符)"));
                }else if( len < 3 && !disabled){
                    callback(new Error("收货人名称不能少于2个字符"));
        ...
        var receiverPhoneNofn=(rule, value, callback) => { //手机验证
            ...
            if(!value && that.rulesmesinfo.delivermode.code==1 && that.rulesmesinfo.installationmode.code==2){
                callback();
        ...
        var addressfn=(rule, value, callback) => { //详细地址验证
            ...
            if(len == 0 && that.rulesmesinfo.delivermode.code==1 && that.rulesmesinfo.installationmode.code==2){
                callback();
            }else if(len < 3 && !addressdisable){
                callback(new Error("请填写完整的详细地址"));
            }else if(len > 80 && !addressdisable){
                callback(new Error("最多80个字符（英语、数字、汉字各占一个字符）"));
            }else if(!/^[0-9a-zA-Z\u4e00-\u9fa5\.()\·\_]+$/.test(value) && !addressdisable){
                callback(new Error('不能输入特殊字符'));
            }else{
                callback();
            }
        ...
        var canChoiceAddressfn=(rule, value, callback) =>{
            ...
            }else if(!addressdisable && !citydisable && !value){
        ...
        var delivertimefn=(rule, value, callback) => { //送货时间验证
            ...
            if(delivertime==2 && !deliverdisable && !value){
        ...
        var installationtimefn=(rule, value, callback) => { //安装时间验证
            ...
            if(installtime==2 && !installdisable && !value){
                ...
                installationtime:{ //安装时间
                    ...
                    pickerOptions: {        //禁用日期
                        disabledDate(time) { // 1operFlag==2 选时间和运能 控制选择日期  2如果送货时间选择了 就以送货时间为当前时间
                            //console.log(this)
                            let cur="";
                            that.rulesmesinfo.delivertime.dateTime?cur=that.rulesmesinfo.delivertime.dateTime:cur=that.rulesmesinfo.archive.installLimitBeginDate
    ...
    methods:{
                ...
                let filesobj={ //提交的信息
                 ...
                 that.$confirm("您确认修改吗?").then(function(){
                    ...
                    if(data && data.success){
                       ...
                       // location.href=env.selectfileURL;
                    }else{
            ...
            API.queryGoodsItemArchive(initobject).then(function(data){
                ...
                //console.log(initdata.archive.addressFlag)
                    ...
                    if(that.operStatu==0){
                        ...
                        if(that.rulesmesinfo.delivermode.code==3  && that.rulesmesinfo.archive.installShowFlag!=0 && !that.rulesmesinfo.installationtime.disabled){
        ...
        initDeliver(isSelected,code,archive){ //取对应的送货参数
            ...
            for(let i=0; i<that.rulesmesinfo.delivermode.options.length;i++){
                if(that.rulesmesinfo.delivermode.options[i][isSelected]==code){
                    ...
                    that.resetinstall(1,isSelected);
                    
                    if(that.rulesmesinfo.delivermode.code==3 && that.rulesmesinfo.archive.installShowFlag!=0 && !that.rulesmesinfo.installationtime.disabled){ //如果售后带货安装提示
                        that.rulesmesinfo.delivermode.message="修改安装时间后自动生成送货时间";
                    }else{
        ...
        resetinstall(s,isSelected){
            ...
            if(!that.rulesmesinfo.delivertime.disabled && isSelected=="code"){ //时间禁止选择的时候不变
            ...
            if(!that.rulesmesinfo.installationtime.disabled && isSelected=="code"){
                that.cancleInstall();
            }
        ...
        installationChang(s){//安装时间选择
            ...
            if(s*1<that.rulesmesinfo.delivertime.dateTime*1){
                ...
                let superability2={ 
                    ...
                    "limittime":{
                        "star":that.rulesmesinfo.archive.deliverySegmentDescBegin,
                        "end":that.rulesmesinfo.archive.deliverySegmentDescEnd,
                        "datatime":that.rulesmesinfo.delivertime.dateTime*1
                    }
                }
                that.superability=superability2;  //重新赋值 触发组件的watch
            }

            if(that.rulesmesinfo.delivermode.code==3){// 如果是售后带货安装的话  安装时间要反写到配送时间
