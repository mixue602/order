...
;(function($) {
  $.fn.accordion = function(options) {
    ...
    var  init = (function(){
        ...
        return (function(){
            $.cookie("compare",_dataCompared.join("|").replace(/\//g,":"),{expires: 30,path: '/',domain:_opt.domain});
    ...
    function makeHTML(data){
        ...
        for(var i = 0;i<_max;i++){
            if(!data[i]){
                ...
                tpl+= '<li class="item-compare"><div class="product"><a href="'+itemInfo.url+'" target="_blank" class="pic"><img src="'+data[i].result.img+'" /></a><p class="name"><a href="'+itemInfo.url+'" target="_blank">'+data[i].result.displayName+'</a></p><p><a href="javascript:void(0)" class="delete" cid="'+data[i].result.searchPrice.productId+'/'+data[i].result.searchPrice.skuId+'">删除</a><em class="price">'+(data[i].result.searchPrice.price?"¥"+data[i].result.searchPrice.price.toFixed(2):"暂无售价")+'</em>'+itemInfo.tags+'</p></div></li>';
                skuData[i] = data[i].result.searchPrice.skuId;
            }
        }
        $("#gm-compareBox-list").empty().html(tpl);
        if(skuData[1] != 0){
            $("#go-commpare").attr("href","//www"+cookieDomain+"/compare/"+skuData.join("-")+".html").attr("target","_blank").removeClass("disable")
        }else{
            $("#go-commpare").attr("href","javascript:void(0)").attr("target","").addClass("disable")
        }
    ...
    $("#gm-compareBox-hide").on("click",function(){
        ...
        $.cookie("g_co","hide",{expires: 30,path: '/',domain:_opt.domain});
    });
    ...
    _item.delegate(_opt.obj,"click",function(){
      ...
      var $item = $(this);
      $.cookie("g_co","show",{expires: 30,path: '/',domain:_opt.domain});
      $("#gm-compareBox").css(_opt.boxStyle).show();
      _dataCompared = $.cookie("compare")?$.cookie("compare").replace(/:/g,"\/").split("|").splice(0,_max) : [];
