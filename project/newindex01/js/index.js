    ...
    ;(function($){
        $(function(){
            ...
            gome_index.prototype = {
                ...
                countTuan:function(){
                    //先向大数据发通知请求，然后不管成功与否都请求秒杀数据接口
                    var that =this;
                    var noticeStatus =false;
                    var distinctId = $.cookie('sensorsdata2015jssdkcross');
                    $.ajax({
                        type: "get",
                        url: "//prom.mobile"+cookieDomain + "/wap/portrait/notify.jsp",
                        data:{"distinctId":distinctId},
                        cache: false,
                        dataType: "jsonp",
                        timeout : 200,
                        jsonpCallback: "getnoticeStatus",
                ...
                tuan:function(){
                    ...
                    var ajaxTimeoutTest=$.ajax({
                        type: "get",
                        url: "//prom.mobile"+cookieDomain+"/wap/promotion/promscms/pchomeRA5OQWzpCIs.jsp",
                        cache: false,
                        dataType: "jsonp",
                        timeout : 200,
                        jsonpCallback: "dynamicActiveTemplet",
                        success: function(tuandata){
                            console.log(tuandata)
                            var data =JSON.parse(tuandata);
                            $('.countdown_js').hide();
                            if(data){
                                $.each(data.templetList, function (i, item) {
                                   //if(item.templetCode == 'dynamicActiveTemplet'){
                                   if(item.templetCode == 'dynamicActiveTemplet'){
                                        var tplData = item.dynamicActiveTemplet.activeList[0];
                                        var endtime=new Date(tplData.rushBuy.endDate).getTime();
                                        /* var curtime=new Date().getTime();*/
                                         var curtime=new Date(data.serverTime).getTime();
                                         ...
                                         if(cookieDomain.indexOf('atguat') !='-1'){
                                            tplData.rushBuy.extra = {
                                                domain: '//tuan.atguat.com.cn'
                                             }
                                         }else{
                                            tplData.rushBuy.extra = {
                                                domain: '//tuan.gome.com.cn'
                                             }
                                         };
                                         
                                         seajs.use('/js/index-tpl', function(template){
                                             var html = template('tuan', tplData.rushBuy);
