...
/**/) {
        'use strict'; var $utils = this, $helpers = $utils.$helpers, indexRushItem = $data.goodsList, $each = $utils.$each, list = $data.list, list_index = $data.list_index, $escape = $utils.$escape, extra = $data.extra, $out = ''; $out += ' ';
        if (indexRushItem) {
            ...
            $each(indexRushItem, function (list, list_index) {
                ...
                if (list_index <= 3) {
                    ...
                    $out += '" target="_blank" title="';
                    $out += $escape(list.skuName);
                    $out += '" data-code="indextuan';
                    $out += $escape(list_index + 1);
                    $out += '"> <img class="lazyloading" src="';
                    $out += $escape(list.skuThumbImgUrl);
                    $out += '" alt="';
                    $out += $escape(list.skuName);
                    $out += '" title="';
                    $out += $escape(list.skuName);
                    $out += '" width="120" height="120" /> <div class="p_price"> <span></span><i>';
                    $out += $escape(list.skuRushBuyPriceDesc);
                    $out += '</i> ';
                    if (list.skuPriceDesc != null) {
                        $out += ' <i class="del">';
                        $out += $escape(list.skuPriceDesc);
                        $out += '</i> ';
                    }
            ...
            if (indexRushItem.length > 4) {
                ...
                $each(indexRushItem, function (list, list_index) {
                    ...
                    if (list_index > 3) {
                        ...
                        $out += '" title="';
                        $out += $escape(list.skuName);
                        $out += '" width="120" height="120" /> <div class="p_price"><i>';
                        $out += $escape(list.skuRushBuyPriceDesc);
                        $out += '</i> ';
                        if (list.skuPriceDesc != null) {
                            $out += ' <i class="del">';
